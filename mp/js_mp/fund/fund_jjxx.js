	...
	$.ajax({
		...
		success: function (data) {
			...
			if (data.retcode == 0000 || data.retcode == "0000") {
				$(".fund_pro_data").html("");
				var fInfoBenchmark=data.data.fInfoBenchmark;
				if(fInfoBenchmark==""||fInfoBenchmark==undefined){
					fInfoBenchmark="-";
				}else{
					fInfoBenchmark=data.data.fInfoBenchmark;
				}
				var fund_pro_data = '<li class="contentlist">' + '<h2>业绩比较基准:</h2><p>' + fInfoBenchmark + '</p>' + '</li>' + '<li class="contentlist">' + '<h2>投资目标:</h2><p>' + data.data.fInfoInvestobject + '</p>' + '</li>' + '<li class="contentlist">' + '<h2>投资范围:</h2><p>' + data.data.fInfoInvestscope + '</p>' + '</li>' + '<li class="contentlist">' + '<h2 >投资理念:</h2><p>' + data.data.fInfoInvestconception + '</p>' + '</li>' + '<li class="contentlist">' + '<h2>决策依据:</h2><p>' + data.data.fInfoDecisionBasis + '</p>' + '</li>';
