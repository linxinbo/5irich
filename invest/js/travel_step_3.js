...
$(function(){
    ...
    if(username == "" || username == null || username == undefined|| username == "null") {
        ...
        var destName1=destName+"×"+travelNum+"人";
        var scene_scheme="每月"+cycle+"号定投"+money+"元";

        //插入旅行计划名称和人数
        $(".planName").html(destName+"×"+travelNum+"人");
        //插入银行卡信息
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.retcode == 0000 || data.retcode == "0000") {
                        //kaiguan=false;
                        $(".timeinvestfund").attr('disabled',"true");
                        saveScene("A",destName1,money,scene_scheme,bank_name,data.data.appsheetserialno,data.data.buyplanno,fundcode,fundname);
    ...
    function saveScene(scene_no,scene_name,scene_money,scene_scheme,debit_account,appsheetserialno,buyplanno,fundcode,fundname){
        $.ajax({
            ...
            success: function (data) {
                if (data.retcode == 0000 || data.retcode == "0000") {
                    console.log("保存计划成功");
                    window.location.href = "travel_success.html?cycle=" + cycle+"&travelTime="+travelTime;
                } else {
