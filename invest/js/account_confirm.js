...
$(document).ready(function () {
    ...
    $.ajax({
        type: "GET",
        url: mainUrl + "invest/data/provinces.json",
        dataType: "json",
        async: true,
        success: function (data) {
            $("#provinces_opaSelect").append('<option value="0" >请选择开户省份</option>');
            $("#cities_opaSelect").append('<option value="0" >请选择城市</option>');
    ...
    //提交开户
    $(".accountsubmit").unbind("click").click(function () {
        openStep3();
    });
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.retcode == 0000) {
                        ...
                        $("#code").hide();
                        showAlertApp("您已在快钱签约，请直接点击下一步进行绑卡！");
                    } else {
                        setErrorMsg(data.retcode, data.retmsg);
                    }
                },
...
function openStep3() {
    ...
    if (smsflag == 1) {
        ...
        $(".accountsubmit").removeClass("btn_next_travel").addClass("btn_next_travel_h");
        $(".accountsubmit").html('正在开户……');
        $.ajax({
            type: "post",
            ...
            success: function (data) {
                ...
                if (data.retcode == 0000) {
                    ...
                    $(".accountsubmit").html('提交开户');
                    $(".accountsubmit").removeClass("btn_next_travel_h").addClass("btn_next_travel");
                    setErrorMsg(data.retcode, data.retmsg);
                }
            },
            error: function (data) {
