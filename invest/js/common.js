...
function setErrorMsg(errorcode, errormsg) {
	if (errormsg == "" || errormsg == null || errormsg == "undefined" || errormsg == undefined) {
		if (errorcode == 1000) {
			showAlertApp("请求参数不合法！");
		}
		if (errorcode == 1001) {
			showAlertApp("您还未登录", gologin);
		} else if (errorcode == 1005) {
			showAlertApp("密码格式错误，为6-16位数字、字母、字符");
		} else if (errorcode == 1006) {
			showAlertApp("手机或邮箱已存在");
		} else if (errorcode == 1007) {
			showAlertApp("您未开通交易账户", gourl);
		} else if (errorcode == 1008) {
			showAlertApp("请求参数为空");
		} else if (errorcode == 1009) {
			showAlertApp("登录名不存在或者密码错误");
		} else if (errorcode == 1010) {
			showAlertApp("您已开户成功或者签约快钱");
		} else if (errorcode == 1011) {
			showAlertApp("给定的手机号或邮箱未注册！");
		} else if (errorcode == 1012) {
			showAlertApp("请先进行风险评估测试", goUrlTest);
		} else if (errorcode == 1020) {
			showAlertApp("手机号格式错误");
		} else if (errorcode == 1004) {
			showAlertApp("验证码失效",goyanz);
		} else if (errorcode == 1023) {
			showAlertApp("银行卡用户名不能为空")
		} else if (errorcode == 1024) {
			showAlertApp("银行卡号不能为空")
		} else if (errorcode == 1025) {
			showAlertApp("银行卡办理网点号不能为空")
		} else if (errorcode == 1031) {
			showAlertApp("用户没有可以赎回的基金");
		} else if (errorcode == 1033) {
			showAlertApp("购买金额小于基金最小值");
		} else if (errorcode == 1034) {
			showAlertApp("基金购买份额太高")
		} else if (errorcode == 1035) {
			showAlertApp("基金状态不能购买")
		} else if (errorcode == 1037) {
			showAlertApp("赎回份额大于用户可用份额")
        } else if (errorcode == 1038) {
			showAlertApp("用户赎回后持有份额小于基金限制最低持有份额，请全部赎回")
        } else if (errorcode == 1039) {
			showAlertApp("用户赎回份额不能小于该基金限制最低赎回份额");
		} else if (errorcode == 1040) {
			showAlertApp("用户赎回份额不能大于该基金限制最高赎回份额")
        } else if (errorcode == 1041) {
			showAlertApp("该基金暂不支持手机购买");
		} else if (errorcode == 1042) {
			showAlertApp("暂无基金详情");
		} else if (errorcode == 911006) {
			showAlertApp("快钱返回：错误的卡号校验位");
		} else if (errorcode == 1000) {
			showAlertApp("请求参数不合法");
		} else {
			showAlertApp(errorcode);
		}
	} else {
		if (errormsg == "未登录") {
			showAlertApp(errormsg, gologin);
		} else {
			showAlertApp(errormsg);
		}
...
//信息提示框类似app的效果
function showAlertApp(str,callback) {
	hideAlertApp();
	var alertBoxMask = $('<div class="alertBoxN"></div>');
	var alertBox;
	var alertBox_h = '';
	alertBox_h += '<div class="alertContentApp">';
	//	alertBox_h += '<h3>提示</h3>';
	alertBox_h += '<div class="alertImg"><img src="images/prompt.png"></div>';
	alertBox_h += '<div class="alertTextApp"><a>'+str+'</a></div>';
	...
	$(".alertContentApp").height($(".alertTextApp").height() + 50);
	var alertMaskH;
	var contH = $(window).width();
	contH = parseInt((contH - 200) / 2);
	$(".alertContentApp").css("left", contH);
	if ($(document).height() >= $(window).height()) {
		alertMaskH = $(document).height();
	} else {
		alertMaskH = $(window).height();
	}
	$(".alertBoxN").height(alertMaskH);
	var alertH = $(".alertContentApp").height();
	$(".alertContentApp").css("margin-top", -Math.ceil(alertH / 2));
	$(".alertBoxN").addClass("disappear");
	setTimeout(function(){
		$(".alertBoxN").remove();
	},1600);
...
function showAlertHint(str, callback, callback2) {
	...
	var alertBoxMask = $('<div class="alertBoxHigh"></div>');

	var alertBox;
	var alertBox_h = '';
	alertBox_h += '<div class="alertContentHigh">';
	alertBox_h += '<h3>'+str+'</h3>';
	/*alertBox_h += '<div class="alertText2 alertTextHint"><span>' + str + '</span></div>';*/
	alertBox_h += '<div class="subButtonHigh"><a class="cancle">取消</a><a class="confirm">确认</a></div>';
	...
	$("body").append(alertBoxMask);
	$(".alertContentHigh").height($(".alertContentHigh h3").height() + 78);
	var alertMaskH;
	var contH = $(window).width();
	contH = parseInt((contH - 250) / 2);
	$(".alertContentHigh").css("left", contH);
	if ($(document).height() >= $(window).height()) {
		alertMaskH = $(document).height();
	} else {
		alertMaskH = $(window).height();
	}
	$(".alertBoxHigh").height(alertMaskH);
	var alertH = $(".alertContentHigh").height();
	$(".alertContentHigh").css("margin-top", -Math.ceil(alertH / 2));
	$(".subButtonHigh .confirm").click(function () {
		console.log("点击确认按钮！");
		if (callback) {
			callback();
		}
	...
	$(".subButtonHigh .cancle").click(function () {
		console.log("点击取消按钮！");
...
//跳转风评
function goUrlTest() {
	window.location.href = mainUrl+"mp/fund/fund_qustList.html";
}
