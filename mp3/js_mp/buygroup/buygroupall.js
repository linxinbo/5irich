...
//购买第一步，风险测试；
function buyGroupStep1(groupid, groupnamea) {
	groupId = groupid;
	...
	$.ajax({
		...
		url: mainUrl + "queryGroupFundInfo", //新的组合购买前置
		data: {
		...
		success: function (data) {
			...
			if (data.retcode == 0000) {
				...
				if (data.data.autRisk == 1014 || data.data.autRisk) {
                    ...
                    buyflag = 1;
					showAlertHint('该基金风险级别高，是否确认购买？', buygogroupnext);
				} else {
					buyflag = 0;
					buygogroupnext();
				}
...
//购买group下一页
function buygogroupnext() {
	window.location.href = mainUrl+"/mp/buygroup/buygroup.html?tano=" + tano + "&comfundlist=" + comfundlist + "&buyflag=" + buyflag + "&groupId=" + groupId +"&groupname="+groupname+"&comratelist="+comratelist;
}
