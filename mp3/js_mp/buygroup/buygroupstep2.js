...
//	alert(fundid)
    ...
    $("#drawBtn").click(function () {
        ...
        if (money%100!==0) {
            showAlert("申购金额不是100的整数倍！");
            return;
        }
        var bankNo = $(".bank_no").parent().find("span").html();
        var transactionaccountid=$(".opaSelect").find("option:selected").attr("data_a");
        if (bankNo == "请选择") {
            showAlert("请选择银行卡");
            return;
        } else{
            var chaniled = $(".bank_no").val();
            window.location.href = "confirmgroup.html?tano="+tano+"&channelid="+chaniled+"&comfundlist="+comfundlist+"&buyflag="+buyflag+"&combinationcode="+fundid+ "&groupname=" +fundname+"&applicationamount="+money+"&moneyaccount="+buygroupC.moneyaccount+"&comratelist="+comratelist+"&fdGroupMin="+buygroupC.x+"&bankno="+buygroupC.depositacct+"&transactionaccountid="+transactionaccountid;
        }
    });
