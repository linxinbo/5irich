	...
	$.ajax({
		...
		success : function(response) {
			...
			if (typeof (result) === "undefined") {
				...
				var principal = 0;
				$("#ma_yetGain span").html(profitLossRatio1 + '%');
				//收益率
				$("#ma_totalVal span").html(formatCurrency(bookMarket));//总资产
				$("#ma_accEarn span").html(formatCurrency(profitLossAmount));//收益
				dataLoading = false;
			} else {
				...
				success : function(response) {
					...
					if (response.retcode == "0000") {
						...
						for ( var i = 0; i < result.length; i++) {
							...
							strHtml += "<div class='moni_cc_list mtopdd'><div class='moni_cc_title'><h2>" + result[i].fundName	+ "（"+result[i].windCode+")</h2></div>";
							strHtml += "<ul class='moni_cc_th'>";
							strHtml += "<li class='l30'><span>交易日期</span><i class='font_gray1'>"+ result[i].startDt +"</i></li>";
							strHtml += "<li class='l30 '><span>交易类型</span><i class='font_gray1'>"+ getTradeType(result[i].tradeType)+ "</i></li>";
							strHtml += "<li class='l30 '><span>交易日净值</span><i class='font_gray1'>"+ fNavUnit.toFixed(4) + "</i></li>";
							strHtml += "<li class='l30 '><span>成交金额</span><i class='font_gray1'>"+ formatCurrency(amountTrade)+ "元</i></li>";
							strHtml += "<li class='l30 '><span>成交份额</span><i class='font_gray1'>"+ shareTrade.toFixed(2) + "份</i></li>";
							strHtml += "<li class='l30 '><span>费率</span><i class='font_gray1'>"+ freeRatio + "</i></li>";
							strHtml += "<li class='l30 '><span>手续费</span><i class='font_gray1'>"+ formatMoney4(free, 4) + "元</i></li>";
