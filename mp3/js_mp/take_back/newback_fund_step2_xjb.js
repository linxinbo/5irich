...
$(document).ready(function () {
    ...
    $("#redeemA").focus(focusf);
    $("#redeemA").blur(blurf);
    $("#newPW").focus(focusf);
    $("#newPW").blur(blurf);

    $("i[data_rest=ID_rest]").click(function(){
        $(this).parent().find("#redeemA").val("");
        $(this).parent().find("#newPW").val("");
        $(this).hide();
    });
    ...
    var per_min_24 = args.per_min_24; //最小可赎回
    ...
    $("#redeemA").attr("placeholder", "可转出金额"+availbal+"元");
    $("#drawBtn").on("click", function () {
        var back_count = $("#redeemA").val();
        var pw = $("#newPW").val();
        if (back_count == "" || back_count == null||parseFloat(back_count)>parseFloat(availbal)) {
            showAlert("赎回金额不能为空或大于可用金额！");
            return false;
        }

        if (pw == "" || pw == null||pw.length<6||pw.length>20) {
            showAlert("交易密码不能为空或格式不对");
            return false;
        }
...
function backStep2(back_count, optionflag, pw) {
    ...
    $.ajax({
        ...
        data: {
            ...
            "redemption.fundcode": fundid,
            "redemption.sharetype": sharetype,
            "redemption.applicationamount": back_count,
            ...
            "redemption.tpasswd": pw,
            "redemption.fastredeemflag":1
        },
        dataType: "JSON",
