...
var refresher = {
	...
	init: function (parameter) {
		...
		scroller.insertBefore(list, scroller.childNodes[0]); //移动！UL的位置到scroll里面。
...
//		this.scrollIt(parameter, pullDownEl, pullDownOffset, "", 40);
	},
	////init函数结束，并且这个文档里面的parameter是传入了一个对象{{
	//	id:"wrapper",
	//pullDownAction:Refresh,
	//pullUpAction:Load
	//}这个函数的功能主要是创建下拉上拉的提示信息，然后对一些变量的初始化。
	//以下这一段应该是对ISCOLL.js的引用。

	scrollIt: function (parameter, pullDownEl, pullDownOffset, pullUpEl, pullUpOffset) {
		myScroll = new iScroll(parameter.id, {
			useTransition: true,
			vScrollbar: false, //hide the iscroll v bar  隐藏掉滑动条
			topOffset: 0,
			onRefresh: function () {
				refresher.onRelease(pullDownEl, pullUpEl);
			}, //调整刷新后的结构。
			onScrollMove: function () { //参数方法判断当前滚动式否到达顶端还是底端
				refresher.onScrolling(this, pullDownEl, pullUpEl, pullUpOffset); //element
			},
			onScrollEnd: function () {//设置动画完成之后执行函数。
	...
	onPulling: function (pullDownEl, pullDownAction, pullUpEl, pullUpAction) {
		if (pullDownEl.className.match('flip')) {
			...
			if (pullDownAction)
				pullDownAction(); // Execute custom function (ajax call?)调用数据
		}
		if (pullUpEl.className.match('flip')) {
			...
			if (pullDownAction);
				pullUpAction(); // Execute custom function (ajax call?)调用数据
		}
	}
}
