...
var m = Math,
	...
	vendor = (function () {
		...
		}//这个应该是判断浏览器是什么内核的，新建的DIV元素，内置的STYLE的属性有t,webkitT,MozT,msT,OT的区别。

		return false;
	})(),
	//返回了这个浏览器的内核属性。
	cssVendor = vendor ? '-' + vendor.toLowerCase() + '-' : '',//如果返回的不是是FALse,则为-xxx-,否则为空。转换为小写字母。
    ...
    // Browser capabilities浏览器适配
	isAndroid = (/android/gi).test(navigator.appVersion),//返回浏览器的平台和版本信息。navigator浏览器的信息。
...
function prefixStyle (style) {
	...
	style = style.charAt(0).toUpperCase() + style.substr(1);//返回第一个style的字母，并大写+之后的字母。例子：asdf>>Asdf
