...
$(function() {
	...
	if (username == "" || username == null || username == undefined	|| username == "null") {
		...
		//$('#fundCode').bind('input propertychange', function() {
		//监控输入
		...
		//console.log(rsgList.dividendsPattern);
		//console.log(mainUrl);			    
		renshengoufeilv();//改变认申购费率

		//load();
		//console.log(rsgList.fundCode);
		//绑定点击提交方法
		...
		//绑定焦点离开方法
		$("#gudingzhideid").blur(function() {
			moneyChange();
		});
		//绑定焦点离开方法
		$("#baifenbideid").blur(function() {
			moneyChange();
		});
	...
	if (renshengou_id_judgment) {
		...
		if (checkFundCode() && !tradeDtError) {
			...
			var params2 = {
				...
				"fundName" : rsgList.fundName,
				"tradeDt" : tradeDt,
				"buyPattern" : rsgList.buyPattern,
				"freeRatio" : freeRatio2,
			...
			console.log(params2);
			$.ajax({
				url : mainUrl + "MutualFundAccountBookBuyAction",
				data : params2,
				dataType : "JSON",
...
function autoCount() {
	...
	if (checkFundCode()) {
		if (rsgList.fundName && rsgList.fundName != "" && rsgList.fundCode
				...
				&& rsgList.amountBuy != "" && rsgList.freeRatio
				&& rsgList.freeRatio != "" && rsgList.freeRatioType
				&& rsgList.freeRatioType != "" && rsgList.tradeDt
				&& rsgList.tradeDt != "") {
			console.log(rsgList.freeRatioType);
			if (rsgList.freeRatioType == "0") {
				freeRatio1 = rsgList.freeRatio / 100;
			...
			console.log(params1);
			$.ajax({
				url : mainUrl + "MutualFundAccountBookBuyPageAction",
				...
				success : function(response_f) {
					if (response_f.retcode == 0000 || response_f.retcode == "0000") {
						...
						rsgList.buyShare = response_f.data.buyShare;
						//console.log(rsgList.buyShare);
						$("#buyShare").val(rsgList.buyShare);
						$("#buyShare").attr("disabled", "disabled");
						...
						console.log("# -----> Data: "
								+ JSON.stringify(rsgList.buyShare));
					} else {
						showAlert("份额返回失败");
					}
				},
...
//获取表单数据
function getrsgdata() {
	rsgList.fundName = $("#fundCode").val(); //基金名称
	rsgList.fundCode = $("#fundCode").val();//基金代码
	rsgList.tradeDt = $("#indate").val(); //交易日期
	rsgList.buyPattern = '0';//认购/申购方式 写死的
	rsgList.freeRatioType = $("#freeRatioType").val();//费率方式 0-百分比 1-固定收费
	rsgList.freeRatio = $("#baifenbideid").val();//申购费率
	//rsgList.freeRatio1 = $("#gudingzhideid").val();//申购费率
	rsgList.amountBuy = $("#amountBuy").val();//购买金额
	rsgList.buyShare = $("#buyShare").val();//购买份额
	rsgList.dividendsPattern = $('#dividendsPattern input[name="dividendsPattern"]:checked').val();//分红方式 0-现金分红 1-分红再投资	
}