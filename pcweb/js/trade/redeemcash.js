    ...
    $('.buyinput').on('blur',function(){
        if($(this).hasClass('redeemshare')){
            if(!redeemformVerify().redeemformredeemshare){
                $(".redeemshare_error1").html("请输入转出金额");
                $(".redeemshare_error").show();
            }else if(!redeemformVerify().redeemformredeemshareS){
                $(".redeemshare_error1").html("最低转出金额为"+per_min_24+"元");
                $(".redeemshare_error").show();
            }else if(!redeemformVerify().redeemformredeemshareD){
                $(".redeemshare_error1").html("最高转出金额为"+per_max_24+"元");
                $(".redeemshare_error").show();
            }else {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.retcode == 0000){
                    kaiguan=false;
                    window.location.href = "redeem_cash_right.html?fundname=" + fundname + "&back_count=" +redeemforminfo.redeemshare;
                }else{
