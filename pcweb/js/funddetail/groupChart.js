...
$(function () {
	...
	$(".dechart ul li:eq(0)").css({color:"#333",fontWeight:"bold"}).siblings("li").css({color:"#666",fontWeight:"normal"});
	//读取收益并绘getFundIncomeData制；
	getGroupIncomeData("CJ1001", 0);
	getGroupIncomeData("CJ1001", 1);
	getGroupIncomeData("CJ1001", 2);
	getGroupIncomeData("CJ1001", 3);
	getGroupIncomeData("CJ1001", 6);
	//切换收益
	$(".dechart ul li").click(function () {
		$(".kmap_date td span").removeClass("selexx");
		$(".kmap_date td span:eq(2)").addClass("selexx");
		$(this).css({color:"#333",fontWeight:"bold"}).siblings("li").css({color:"#666",fontWeight:"normal"});
		/**  点击切换收益种类**/
		if ($(this).index() != 1) {
	...
	$(".kmap_date td span").unbind("click").click(function () {
		...
		if (incomeType == 0) {
			if (_index == 0) {
				...
				//$(".td_value_2").html("");
				//$(".td_value_21").html("");
				//$(".td_value_21").html("近一个月收益");
				//$(".td_value_2").append(mydata.x1[mydata.x1.length-1]+"%");
			} else if (_index == 1) {
				...
				//$(".td_value_2").html("");
				//$(".td_value_21").html("");
				//$(".td_value_21").html("近三个月收益");
				//$(".td_value_2").append(mydata.x3[mydata.x3.length-1]+"%");
			} else if (_index == 2) {
				newchart.option.xAxis.tickInterval = 30;
				newchart.option.xAxis.categories = mydata.y6;
				//				newchart.option.series[0].data=mydata.y;
				...
				//$(".td_value_2").html("");
				//$(".td_value_21").html("");
				//$(".td_value_21").html("近六个月收益");
				//$(".td_value_2").append(mydata.x6[mydata.x6.length-1]+"%");
			} else if (_index == 3) {
				...
				//$(".td_value_2").html("");
				//$(".td_value_21").html("");
				//$(".td_value_21").html("近一年收益");
				//$(".td_value_2").append(mydata.xYear[mydata.xYear.length-1]+"%");
			}
			else if (_index == 6) {
				newchart.option.xAxis.tickInterval = 80;
				...
				//$(".td_value_21").html("");
				//$(".td_value_21").html("近一年收益");
				//$(".td_value_2").append(mydata.xall[mydata.xall.length-1]+"%");
			}
		} else {
			if (_index == 0) {
				newchart.option.xAxis.tickInterval=6;
		...
		success: function (data) {
			...
			//	hideloading();
				if (date == 0) {
					...
					//净值收益
					mydata.xall = data.data.yfnavTurnRatioList;
					mydata.yall = data.data.xList;
					mydata.aall = data.data.yfnavUnitList;
					//沪深300；
					mydata.nall = data.data.ysDqCloseList;
					setprecentDataType(mydata.xall);
					setprecentDataType(mydata.nall);
