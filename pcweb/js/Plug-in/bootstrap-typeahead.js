...
!function($) {
	...
	Typeahead.prototype = {
		...
		grepper : function(data) {
			...
			if (data && data.length
					&& !data[0].hasOwnProperty(that.options.display)) {
				return null;
			}
		...
		sorter : function(items) {
			...
			while (item = items.shift()) {
				if (!item[that.options.display].toLowerCase().indexOf(
			...
			items = $(items).map(
					function(i, item) {
						i = $(that.options.item).attr({
							'data-value' : item[that.options.val],
							'data-netValue' : item[that.options.netValue]
						});
						i.find('a').html(that.highlighter(item[that.options.val],item));
						return i[0];
					});
		...
		select : function() {
			...
			this.options.itemSelected($selectedItem, $selectedItem
					.attr('data-value'), $selectedItem.text(), $selectedItem
					.attr('data-netValue'));
			return this.hide();
		},
		...
		//
		listen : function() {
			this.$element.on('blur', $.proxy(this.blur, this)).on('keyup',
